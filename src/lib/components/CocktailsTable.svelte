<script lang="ts">
	...
	export let initialSelectedTags: Tag[] = [];
	export let initialSelectedIngredients: Ingredient[] = [];
	export let initialLogicMode: LogicMode = 'AND';

	let searchTerm = initialSearchTerm;
	let selectedTags: Tag[] = initialSelectedTags;
	let selectedIngredients: Ingredient[] = initialSelectedIngredients;
	let logicMode: LogicMode = initialLogicMode;
	...
	$: filteredCocktails = cocktails.filter((cocktail) => {
		...
		const matchesSearch = showSearch ? matchesTitle || matchesDescription || matchesVariants : true;

		const matchesTags = showFilters ? matchesTagsLogic(cocktail, selectedTags, logicMode) : true;
		const matchesIngredients = showFilters
			? matchesIngredientsLogic(cocktail, selectedIngredients, logicMode)
			: true;
...
{#if showFilters && (selectedTags.length > 0 || selectedIngredients.length > 0)}
					...
					<div>
						<div class={gridClass}>
							...
							<div class={nameColClass}>
								...
								<p class="text-gray-600 text-sm leading-relaxed mt-1 md:hidden mobile-line-clamp-2">
										...
										<div class={gridClass}>
											...
											<div class={nameColClass}>
												...
												{#if formatVariantIngredients(variant)}
													<p class="text-gray-400 italic text-xs leading-relaxed mt-0.5 md:hidden">
